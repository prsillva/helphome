...
def loginpwa(request):
    if request.method == "POST":
        ...
        form_login = AuthenticationForm()
    return render(request, 'core/login_pwa.html', {'form_login': form_login})
...
@login_required(login_url='/loginpwa')
def logoutpwa(request):
    logout(request)
    return redirect('pwa')
...
def profile(request):
    ...
    return render(request, 'core/pwa.html', context)
